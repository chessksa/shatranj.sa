...
MIGRATION_PATH = ROOT / 'supabase/migrations/20260909210000_tournament_round_break.sql'

MIGRATION = r'''-- Three-minute inter-round break for knockout tournaments.
...
page = replace_once(
    page,
    ".tournament-enter-btn{min-height:36px;min-width:112px;font-size:12px}.bracket-empty",
    ".tournament-enter-btn{min-height:36px;min-width:112px;font-size:12px}.tournament-enter-btn.round-break-countdown{min-width:158px;opacity:1!important;background:rgba(216,182,101,.14);color:var(--hero-gold-2);cursor:default;box-shadow:none}.bracket-empty",
    'round-break countdown style',
)

page = replace_once(
    page,
    "let tournamentMatchPollTimer=null;",
    "let tournamentMatchPollTimer=null;\nlet tournamentRoundCountdownTimer=null;\nlet tournamentBracketRefreshTimer=null;",
    'round timer state',
)

old_helpers = "function roundLabel(round,maxRound){if(round===maxRound)return 'النهائي';if(round===maxRound-1)return 'نصف النهائي';if(round===maxRound-2)return 'ربع النهائي';return `الدور ${round}`}\n\nasync function loadTournamentBracket"
new_helpers = """function roundLabel(round,maxRound){if(round===maxRound)return 'النهائي';if(round===maxRound-1)return 'نصف النهائي';if(round===maxRound-2)return 'ربع النهائي';return `الدور ${round}`}
function formatTournamentRoundRemaining(ms){const total=Math.max(0,Math.ceil(Number(ms||0)/1000));const minutes=Math.floor(total/60);const seconds=total%60;return `${String(minutes).padStart(2,'0')}:${String(seconds).padStart(2,'0')}`}
function updateTournamentRoundCountdowns(){
  document.querySelectorAll('[data-round-open-at]').forEach(button=>{
    const opensAt=Date.parse(button.dataset.roundOpenAt||'');
    if(!Number.isFinite(opensAt))return;
    const remaining=Math.max(0,opensAt-Date.now());
    if(remaining<=0){
      button.classList.remove('round-break-countdown');
      button.textContent='دخول المباراة';
      button.removeAttribute('data-round-open-at');
      if(button.dataset.roundPolling!=='1')button.disabled=false;
      return;
    }
    button.textContent=`الدور التالي خلال ${formatTournamentRoundRemaining(remaining)}`;
  });
}
function ensureTournamentRoundCountdowns(){clearInterval(tournamentRoundCountdownTimer);updateTournamentRoundCountdowns();tournamentRoundCountdownTimer=setInterval(updateTournamentRoundCountdowns,1000)}
function scheduleTournamentBracketRefresh(){clearInterval(tournamentBracketRefreshTimer);if(!selectedTournamentId)return;tournamentBracketRefreshTimer=setInterval(()=>{if(selectedTournamentId)loadTournamentBracket(selectedTournamentId)},5000)}

async function loadTournamentBracket"""
page = replace_once(page, old_helpers, new_helpers, 'countdown helpers')

old_render = """    host.className='';
    host.innerHTML=[...groups.entries()].map(([round,matches])=>`<section class=\"bracket-round\"><h4 class=\"bracket-round-title\">${esc(roundLabel(round,maxRound))}</h4><div class=\"bracket-grid\">${matches.map(match=>{const canEnter=Boolean(match.is_my_match&&match.player_one_id&&match.player_two_id&&['pending','active'].includes(match.match_status));const canWatch=Boolean(match.match_status==='active'&&match.game_id);const retry=Number(match.attempt_no||1)>1?` · إعادة ${Number(match.attempt_no)}`:'';return `<article class=\"bracket-match${match.is_my_match?' mine':''}\"><div class=\"bracket-player${match.winner_player_id===match.player_one_id?' winner':''}\"><span>${esc(match.player_one_name||'بانتظار المتأهل')}</span></div><div class=\"bracket-player${match.winner_player_id===match.player_two_id?' winner':''}\"><span>${esc(match.player_two_name||'بانتظار المتأهل')}</span></div><div class=\"bracket-meta\"><span>${esc(bracketStatusLabel(match.match_status))}${retry}</span>${canEnter?`<button class=\"register-btn tournament-enter-btn\" type=\"button\" data-tournament-match=\"${esc(match.match_id)}\">دخول المباراة</button>`:''}${canWatch?`<a class=\"register-btn tournament-watch-btn\" href=\"play.html?spectate=${encodeURIComponent(match.game_id)}\">مشاهدة</a>`:''}</div></article>`}).join('')}</div></section>`).join('');
"""
new_render = """    host.className='';
    host.innerHTML=[...groups.entries()].map(([round,matches])=>`<section class=\"bracket-round\"><h4 class=\"bracket-round-title\">${esc(roundLabel(round,maxRound))}</h4><div class=\"bracket-grid\">${matches.map(match=>{
      const participant=Boolean(match.is_my_match&&match.player_one_id&&match.player_two_id&&['pending','active'].includes(match.match_status));
      const readyAt=match.ready_opens_at?Date.parse(match.ready_opens_at):NaN;
      const roundBreak=Boolean(participant&&match.match_status==='pending'&&Number(match.round_no)>1&&(!match.ready_opens_at||(Number.isFinite(readyAt)&&readyAt>Date.now())));
      const canEnter=participant&&!roundBreak;
      const canWatch=Boolean(match.match_status==='active'&&match.game_id);
      const retry=Number(match.attempt_no||1)>1?` · إعادة ${Number(match.attempt_no)}`:'';
      let entry='';
      if(roundBreak){
        const attrs=match.ready_opens_at?` data-round-open-at=\"${esc(match.ready_opens_at)}\"`:'';
        const label=match.ready_opens_at?`الدور التالي خلال ${formatTournamentRoundRemaining(readyAt-Date.now())}`:'بانتظار اكتمال الدور الحالي';
        entry=`<button class=\"register-btn tournament-enter-btn round-break-countdown\" type=\"button\" data-tournament-match=\"${esc(match.match_id)}\"${attrs} disabled>${esc(label)}</button>`;
      }else if(canEnter){
        entry=`<button class=\"register-btn tournament-enter-btn\" type=\"button\" data-tournament-match=\"${esc(match.match_id)}\">دخول المباراة</button>`;
      }
      return `<article class=\"bracket-match${match.is_my_match?' mine':''}\"><div class=\"bracket-player${match.winner_player_id===match.player_one_id?' winner':''}\"><span>${esc(match.player_one_name||'بانتظار المتأهل')}</span></div><div class=\"bracket-player${match.winner_player_id===match.player_two_id?' winner':''}\"><span>${esc(match.player_two_name||'بانتظار المتأهل')}</span></div><div class=\"bracket-meta\"><span>${esc(bracketStatusLabel(match.match_status))}${retry}</span>${entry}${canWatch?`<a class=\"register-btn tournament-watch-btn\" href=\"play.html?spectate=${encodeURIComponent(match.game_id)}\">مشاهدة</a>`:''}</div></article>`;
    }).join('')}</div></section>`).join('');
    ensureTournamentRoundCountdowns();
"""
page = replace_once(page, old_render, new_render, 'bracket round rendering')

old_active = """      if(row?.state==='active'&&row.game_id&&row.seat_key&&['w','b'].includes(row.color)){
        sessionStorage.setItem('shatranj_live_game_id',row.game_id);"""
new_active = """      if(row?.state==='round_break'){
        button.disabled=true;
        button.classList.add('round-break-countdown');
        button.dataset.roundPolling='1';
        if(row.ready_opens_at){button.dataset.roundOpenAt=row.ready_opens_at;updateTournamentRoundCountdowns()}else{button.removeAttribute('data-round-open-at');button.textContent='بانتظار اكتمال الدور الحالي'}
        tournamentMatchPollTimer=setTimeout(poll,1500);
        return;
      }
      if(row?.state==='active'&&row.game_id&&row.seat_key&&['w','b'].includes(row.color)){
        sessionStorage.setItem('shatranj_live_game_id',row.game_id);"""
page = replace_once(page, old_active, new_active, 'access round-break handling')
...
page = replace_once(
    ...
    "      delete button.dataset.roundPolling;\n      button.classList.remove('round-break-countdown');\n      button.removeAttribute('data-round-open-at');\n      button.textContent='انتهت المباراة';",
...
old_nav = "function openTournamentDetail(tournamentId){const row=tournaments.find(item=>String(item.id)===String(tournamentId));if(!row)return;selectedTournamentId=String(row.id);setRegistrationMessage('');renderTournamentDetail(row);listView.hidden=true;detailView.hidden=false;loadTournamentBracket(row.id)}\nfunction showTournamentList(){clearTimeout(tournamentMatchPollTimer);selectedTournamentId=null;setRegistrationMessage('');detailView.hidden=true;listView.hidden=false}"
new_nav = "function openTournamentDetail(tournamentId){const row=tournaments.find(item=>String(item.id)===String(tournamentId));if(!row)return;selectedTournamentId=String(row.id);setRegistrationMessage('');renderTournamentDetail(row);listView.hidden=true;detailView.hidden=false;loadTournamentBracket(row.id);if(row.status==='running')scheduleTournamentBracketRefresh()}\nfunction showTournamentList(){clearTimeout(tournamentMatchPollTimer);clearInterval(tournamentRoundCountdownTimer);clearInterval(tournamentBracketRefreshTimer);selectedTournamentId=null;setRegistrationMessage('');detailView.hidden=true;listView.hidden=false}"
page = replace_once(page, old_nav, new_nav, 'detail refresh lifecycle')

PAGE_PATH.write_text(page, encoding='utf-8')
print('tournament round-break implementation applied')
