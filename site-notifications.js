(async () => {
  ...
  function targetFor(n) {
    if (n.type === 'challenge_accepted' && n.game_id && n.challenge_id) {
      return `play-v10.html?game=${encodeURIComponent(n.game_id)}&challenge=${encodeURIComponent(n.challenge_id)}`;
    }
  ...
  async function respondToChallenge(button) {
    ...
    try {
      ...
      location.href = `play-v10.html?game=${encodeURIComponent(game.game_id)}&challenge=${encodeURIComponent(challengeId)}`;
    } catch (error) {
