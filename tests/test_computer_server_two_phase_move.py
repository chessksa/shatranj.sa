...
assert move_block.index('await persistPlayerTurnBeforeComputer') < move_block.index('chooseComputerMove'), 'persist player move before engine selection/wait'
assert 'completePendingComputerTurn' in EDGE, 'server needs a resumable pending-computer-turn helper'
assert 'chess.turn() === \'b\'' in state_block and 'completePendingComputerTurn' in state_block, 'state reconciliation must resume a pending computer turn instead of returning the pre-move position'
