...
begin
  ...
  if v_tournament.registration_opens_at is not null
     and clock_timestamp() < v_tournament.registration_opens_at then
    return jsonb_build_object('ok', false, 'code', 'registration_closed');
  end if;

  if v_tournament.registration_closes_at is not null
     and clock_timestamp() >= v_tournament.registration_closes_at then
    return jsonb_build_object('ok', false, 'code', 'registration_closed');
  end if;

  v_player_country := coalesce(
    nullif(btrim(v_player.country), ''),
    nullif(btrim(v_player.region), '')
  );
