-- Keep the global signup/rating pool open while excluding one country from nationality and residence.

alter table public.players
  drop constraint if exists players_excluded_country_check;

alter table public.players
  add constraint players_excluded_country_check
  check (
    not (
      lower(btrim(coalesce(region,''))) in ('israel','il','state of israel')
      or btrim(coalesce(region,'')) in ('إسرائيل','اسرائيل','ישראל')
      or lower(btrim(coalesce(country,''))) in ('israel','il','state of israel')
      or btrim(coalesce(country,'')) in ('إسرائيل','اسرائيل','ישראל')
    )
  );
...
begin
  ...
  if lower(v_nationality) in ('israel','il','state of israel')
     or v_nationality in ('إسرائيل','اسرائيل','ישראל')
     or lower(v_residence_country) in ('israel','il','state of israel')
     or v_residence_country in ('إسرائيل','اسرائيل','ישראל') then
    raise exception 'blocked country';
  end if;
