...
const isAdvancedVariant = (value: unknown): value is AdvancedVariant =>
  ADVANCED_VARIANTS.includes(String(value) as AdvancedVariant);
...
function positionFromFen(variant: AdvancedVariant, fen: string): Position {
  ...
  return setupPosition(ruleFor(variant), setup).unwrap();
}
...
function legalMoves(pos: Position): string[] {
  ...
  for (const [from, dests] of pos.allDests(ctx)) {
    for (const to of dests) {
      ...
      if (promotes) {
        for (const promotion of promotionRoles) moves.push(makeUci({ from, to, promotion }));
      } else {
...
function pocketsFor(pos: Position) {
  ...
  const side = (color: 'white' | 'black') => ({
    pawn: pos.pockets![color].pawn,
    knight: pos.pockets![color].knight,
    bishop: pos.pockets![color].bishop,
    rook: pos.pockets![color].rook,
    queen: pos.pockets![color].queen,
  });
...
function resultFor(pos: Position) {
  ...
  const result = outcome.winner === 'white' ? '1-0'
    : outcome.winner === 'black' ? '0-1'
    : '1/2-1/2';
  return { result, termination: `variant:${pos.rules}` };
}
...
Deno.serve(async (req: Request) => {
  ...
  const safeState = async (game: Record<string, any>) => {
    ...
    return {
      game: {
        ...
        white_name: white?.name || 'الأبيض',
        black_player_id: game.black_player_id,
        black_name: black?.name || 'الأسود',
        my_color: game.white_player_id === player.id ? 'w' : 'b',
        base_seconds: game.base_seconds,
        increment_seconds: game.increment_seconds,
        white_ms: game.white_ms,
        black_ms: game.black_ms,
        clock_anchor_at: game.clock_anchor_at,
      ...
      legalMoves: game.status === 'active' ? legalMoves(pos) : [],
      pockets: pocketsFor(pos),
  ...
  if (action === 'queue') {
    ...
    if (!isAdvancedVariant(variant)) return reply({ error: 'Unsupported variant' }, 400);
  ...
  try { game = await loadGame(gameId); }
  catch (error) {
    const code = error instanceof Error ? error.message : 'game_not_found';
    return reply({ error: code }, code === 'game_not_accessible' ? 403 : 404);
  }
  ...
  if (['resign', 'offer_draw', 'respond_draw', 'timeout'].includes(action)) {
    const { data, error } = await admin.rpc('v3_variant_action_server', {
      ...
      p_player_id: player.id,
      p_action: action,
      p_accept: action === 'respond_draw' ? Boolean(body.accept) : null,
    });
    if (error) return reply({ error: 'تعذر تنفيذ الإجراء', code: error.message }, 409);
    return reply({ ...(await safeState(data)), serverNow: new Date().toISOString() });
  }
  ...
  if (!Number.isInteger(expectedPly) || expectedPly < 0 || game.ply !== expectedPly || !uci) {
    return reply({ error: 'Game state changed', code: 'stale_state' }, 409);
  }

  const mover = game.white_player_id === player.id ? 'w' : 'b';
  ...
  if ((mover === 'w' ? whiteMs : blackMs) <= 0) {
    const timed = await admin.rpc('v3_variant_action_server', {
      p_game_id: gameId, p_player_id: player.id, p_action: 'timeout', p_accept: null,
    });
    return reply({ game: timed.data ?? game, error: timed.error ? 'Clock expired' : undefined }, 409);
  }

  let pos: Position;
  let move: Move | undefined;
  try {
    pos = positionFromFen(game.variant as AdvancedVariant, game.fen);
    move = parseUci(uci);
  } catch {
  ...
  if (mover === 'w') whiteMs += incrementMs;
  else blackMs += incrementMs;
  const { result, termination } = resultFor(pos);
  ...
  const { data: committed, error: commitError } = await admin.rpc('v4_commit_advanced_variant_move_server', {
    ...
    p_uci: makeUci(move),
    p_fen: newFen,
    p_next_turn: turnCode(pos.turn),
    p_white_ms: Math.round(whiteMs),
    p_black_ms: Math.round(blackMs),
    p_result: result,
